...
/// Line-control: bit 7 selects the baud-rate register view.
const LCR_DLAB: u8 = 0x80;
/// Interrupt-enable: data-available interrupt.
const IER_ELSI: u8 = 0x01;
...
pub struct Serial {
    ...
    lcr: u8,
    ier: u8,
    /// Only the loopback bit is kept: there is one FIFO depth, and the
    /// reset bits are self-clearing pulses applied on write.
    fcr: u8,
    mcr: u8,
    scr: u8,
...
impl Serial {
    ...
    /// True when the driver wants a data-available interrupt: ELSI set in
    /// IER and OUT2 set in MCR (the UART's master interrupt gate).
    fn rx_irq_armed(&self) -> bool {
        (self.ier & IER_ELSI) != 0 && (self.mcr & MCR_OUT2) != 0
    }

    /// True when either loopback gate is set: MCR bit 4 or FCR bit 4.
    fn loopback(&self) -> bool {
        (self.mcr & MCR_LOOP) != 0 || (self.fcr & FCR_LOOP) != 0
    }
    ...
    fn reconcile_line(&mut self) {
        if let Some(want) = self.pending_line.take() {
            if self.irq.set_level(self.gsi, want).is_ok() {
                self.raised = want;
            } else {
                self.pending_line = Some(want);
            }
    ...
    fn read_byte(&mut self, offset: u64) -> u8 {
        ...
        match offset {
            ...
            1 => self.ier,
            2 => {
                // Reading IIR does not consume the receive FIFO.
            ...
            3 => 0, // LCR reads as 0 on the part.
    ...
    fn write_byte(&mut self, offset: u64, value: u8) {
        ...
        match offset {
            ...
            1 => {
                self.ier = value & IER_ELSI; // only data-available is real
            ...
            2 => {
                ...
                self.fcr = value & FCR_LOOP;
            }
            // Intentionally no parity generation: the BoxLite console is
            // always 8N1, so WLS/STB/PEN/EPAR are stored but never acted on.
...
mod tests {
    ...
    fn dlab_switches_the_baud_and_data_views() {
        ...
        port.write(3, 0x03);
        // DLAB is off when offset 0 serves RBR again, and LCR reads as 0.
        assert_eq!(port.read(3), 0x00);
        assert_eq!(port.read(0), 0); // RBR, empty — not the divisor latch
    }

    #[test]
    fn ier_keeps_only_the_data_available_bit() {
        let mut port = port();
        port.write(1, 0x0F);
    ...
    fn received_byte_raises_level_irq_until_drained() {
        ...
        assert_eq!(port.read(2), IIR_RECEIVED_DATA);
        assert_eq!(port.read(2), IIR_RECEIVED_DATA, "IIR reads keep the byte");
    ...
    fn arming_after_buffering_raises_immediately() {
        let mut port = port();
        port.write(3, 0x03);
        port.write(1, 0x01); // ELSI while still no OUT2
    ...
    fn unarmed_port_buffers_without_raising() {
        ...
        assert!(port.calls().is_empty());
        assert_eq!(port.read(5) & LSR_DATA_READY, LSR_DATA_READY);
        assert_eq!(port.read(2), IIR_NO_INTERRUPT);
    }
    ...
    #[test]
    fn fcr_loopback_bit_arms_the_same_route() {
        let mut port = port();
        port.write(2, FCR_LOOP);
        port.write(0, b'w');
        assert!(port.sink.0.lock().unwrap().is_empty());
        assert_eq!(port.read(0), b'w');
    }
    ...
    fn wide_accesses_walk_registers_without_panic() {
        ...
        assert_eq!(data, [0, 0x01, IIR_NO_INTERRUPT, 0]);
        // A two-byte write spans THR and IER: only 'p' reaches the sink;
        // 'q' lands in IER and its ELSI bit keeps the armed state.
