...
Generated TypeScript calls keep a single optional `ListOrganizationsOptions` argument: `listOrganizations()`, `listOrganizations(options)` and `listOrganizations({ ...options, referredCode: code })`. Existing Axios options-only callers remain compatible; only `referredCode` becomes a query parameter. The TypeScript postprocessor preserves this contract on regeneration. Generate with `make generate:apps:referral`, then run the contract/type checks.
...
| --- | --- | --- |
| `BUSINESS_EVENTS_ENABLED` | `false` | Explicit `true` or `false`; independent of usage export |
...
All numeric settings are positive safe integers. `ceil(batchSize / concurrency) * timeoutMs + 5000` must be **less** than visibilityMs. Publishing defaults to disabled in both the API and infrastructure; registration still records pending events. Set `BUSINESS_EVENTS_ENABLED=true` after configuring Commerce. Enabled but missing/invalid connection settings fails API startup, after Nest loads `.env`. The native local launcher also defaults publishing off for older `.env` files, while preserving explicit settings. Infrastructure forwards these settings independently from the existing usage-export switch.
...
The default API unit-test job runs with event publishing disabled by default and excludes the referral database and Commerce suites. The integration and acceptance make targets use `api/jest.referral.config.ts` explicitly; missing service dependencies still fail those targets.
...
3. Deploy **all** BoxLite API replicas with valid Publisher configuration and explicitly set `BUSINESS_EVENTS_ENABLED=true`; verify the new GET contract.
