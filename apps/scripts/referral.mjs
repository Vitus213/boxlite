...
const base = execFileSync('git', ['merge-base', process.env.REFERRAL_BASE ?? 'origin/main', 'HEAD'], {
  cwd: apps,
  encoding: 'utf8',
}).trim()
const changed = [
  ...execFileSync('git', ['diff', base, '--name-only', '--diff-filter=ACMRTUXB'], { cwd: apps, encoding: 'utf8' })
    .trim()
    .split('\n'),
...
async function run(command, args) {
  if (args[0] === 'eslint' && args.length === 1) return
  if (args[0] === 'prettier' && !changed.length) return
